...
maxlen = 20 * vec_size # 每个评论我们只考虑100个单词
    ...
    for time in sentence_list[i]['times']:
        for attribute in sentence_list[i]['attributes']:
            for value in sentence_list[i]['values']:
                for words in range(len(sentence_list[i]['words'])):
                    if words == 0:
                        x_train[train_num] = model.wv[sentence_list[i]['words'][words]].tolist()
                    else:
                        x_train[train_num] += model.wv[sentence_list[i]['words'][words]].tolist()
                y_train[train_num] = int([time, attribute, value] in sentence_list[i]['results'])
                x_train[train_num] += [time, attribute, value]
                train_num+=1
                #if(y_train[train_num-1] == 1):
                 #   for j in range(13):
    ...
    for time in sentence_list[i]['times']:
        for attribute in sentence_list[i]['attributes']:
            for value in sentence_list[i]['values']:
                for words in range(len(sentence_list[i]['words'])):
                    if words == 0:
                        x_test[test_num] = model.wv[sentence_list[i]['words'][words]].tolist()
                    else:
                        x_test[test_num] += model.wv[sentence_list[i]['words'][words]].tolist()
                x_test[test_num] += [time, attribute, value]
...
x_train_pad = sequence.pad_sequences(x_train, maxlen=maxlen, dtype='float')
x_test_pad = sequence.pad_sequences(x_test, maxlen=maxlen, dtype='float') #长了就截断，短了就补0

x_train_pad -= x_train_pad.min()
x_test_pad -= x_test_pad.min()
...
old_model = load_model('my_model_word2vec.h5')

#plot_acc_and_loss(history)
preds = (model.predict(x_test_pad) > 0.5).astype("int32")
old_preds = (old_model.predict(x_test_pad) > 0.5).astype("int32")
...
if f1 > old_f1:
    model.save('my_model_word2vec.h5')
    old_p = p
    old_r = r
    old_f1 = f1
